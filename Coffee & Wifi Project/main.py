from flask import Flask, render_template
from flask_bootstrap import Bootstrap5
from flask_wtf import FlaskForm
from wtforms import StringField, SubmitField, SelectField
from wtforms.validators import DataRequired, URL
import csv

# This is for displaying local cafe info

app = Flask(__name__)
app.secret_key = '8BYkEfBA6O6donzWlSihBXox7C0sKR6b'
Bootstrap5(app)


class CafeForm(FlaskForm):
    cafe = StringField('Cafe name', validators=[DataRequired()])
    location = StringField('Cafe Location On Google Map (URL)', validators=[DataRequired(), URL()])
    open_time = StringField('Opening Time e.g 8AM', validators=[DataRequired()])
    close_time = StringField('Closing Time e.g 5:30PM', validators=[DataRequired()])
    coffee_choices = ['☕', '☕☕', '☕☕☕', '☕☕☕☕', '☕☕☕☕☕']
    coffee_rating = SelectField('Coffee Rating', choices=coffee_choices)
    wifi_strength_choices = ['✘', '💪', '💪💪', '💪💪💪', '💪💪💪💪', '💪💪💪💪💪']
    wifi_rating = SelectField('Wifi Strength Rating', choices=wifi_strength_choices)
    power_socket_choices = ['✘', '🔌', '🔌🔌', '🔌🔌🔌', '🔌🔌🔌🔌', '🔌🔌🔌🔌🔌']
    power_socket = SelectField('Power Socket Availability', choices=power_socket_choices)
    submit = SubmitField('Submit')


# ---------------------------------------------------------------------------


# all Flask routes below
@app.route("/")
def home():
    return render_template("index.html")


@app.route('/add', methods=['GET', 'POST'])
def add_cafe():
    form = CafeForm()
    form_data = [
        form.cafe.data,
        form.location.data,
        form.open_time.data,
        form.close_time.data,
        form.coffee_rating.data,
        form.wifi_rating.data,
        form.power_socket.data
    ]

    if form.validate_on_submit():
        with open('cafe-data.csv', 'a', newline='', encoding='utf-8') as data:
            writer = csv.writer(data)
            line = form_data
            writer.writerow(line)
    return render_template('add.html', form=form)


@app.route('/cafes')
def cafes():
    with open('cafe-data.csv', newline='', encoding='utf-8') as csv_file:
        csv_data = csv.reader(csv_file, delimiter=',')
        list_of_rows = []
        for row in csv_data:
            list_of_rows.append(row)
    return render_template('cafes.html', cafes=list_of_rows)


if __name__ == '__main__':
    app.run(debug=True)
